...
5개 섹션(핵심 개념, 개념 설명, 핵심 요약, 흔한 오해, 실용적 응용)으로
...
- 아래 지정된 5개 섹션 형식을 정확히 따르세요.
- 개념 설명은 충분히 상세하게 작성하고, 반드시 구체적인 코드 예시나 실제 사용 예시를 포함하세요."""

# 출력 형식: 5개 섹션 학습 가이드 구조
...
위의 각 핵심 개념을 아래 형식으로 상세히 설명하세요.
각 개념마다 반드시 코드 예시 또는 구체적인 사용 예시를 포함하세요.

**개념명**
- 설명: 개념의 의미와 동작 방식을 3~5문장으로 자세히 설명하세요.
- 예시:
  ```python
  # 구체적인 코드 예시 또는 실제 사용 예시
  ```
...
### 5. 실용적 응용 (Practical Applications)
강의에서 다룬 개념들이 실제 백엔드 개발에서 어떻게 활용되는지 구체적인 예시를 들어 설명하세요.
- 활용 예시1
- 활용 예시2
---"""
...
위 강의 내용을 바탕으로 아래 5개 섹션 형식에 맞게 한국어로 학습 가이드를 작성하세요:
""" + _GUIDE_OUTPUT_FORMAT
...
def get_study_guide_prompt() -> ChatPromptTemplate:
    ...
    5개 섹션으로 구성된 한국어 학습 가이드를 생성하는 프롬프트입니다.
