...
1. <thinking> 태그 안에서 아래 항목을 순서대로 분석하세요:
   ...
   **[STEP 2] 문항 설계 (중복 방지 + 정답 배분 계획)**
   ...
   - MCQ 정답 배분 사전 계획: A, B, C, D가 각각 최소 1회 이상 등장하고 어떤 선택지도 전체 MCQ의 절반 이상 차지하지 않도록 배분 (예: MCQ 6문항이면 각 최대 2회)
   - 마지막에 "정답 분포 확인: A=N회, B=N회, C=N회, D=N회" 체크
...
- 오답 함정 필수 규칙:
  ...
  · 여러 선택지를 묶어서 "A, C, D는 관련 없다" 형식 절대 금지 — 각각 개별 설명 필수
...
{{
  "quizzes": [
    {{
      ...
      "answer": "B",
      "explanation": "✅ 정답 근거: [강의에 근거한 정답 이유] | ❌ 오답 함정: A-[A의 실제 개념 설명, 왜 정답이 아닌지], C-[C의 실제 개념 설명, 왜 정답이 아닌지], D-[D의 실제 개념 설명, 왜 정답이 아닌지]"
    }},
    {{
      "type": "short_answer",
      "style": "comparison",
